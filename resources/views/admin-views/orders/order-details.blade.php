...
<div class="toast-container mt-5 pt-5 position-absolute top-0 end-0 p-3" style="right: unset!important" dir="rtl">
    @if(session("success"))
            ...
            <div class="row mb-5 pb-4">
                ...
                <p class="mt-5 text-dark fw-bold">
                    ...
                    <span class="text-muted ms-3">
                        <a href="/admin/user/{{$order->owner()->first()->id}}/show">
                ...
                <p class="mt-3 text-dark fw-bold">
                    ...
                    <span class="text-muted ms-3">
                        <a href="/admin/user/{{$order->supervisor()->first()->id}}/show">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <th class="py-2" scope="row">
                                <a href="/admin/product/{{ $present->product_id }}/show" class="text-primary fw-bold fs-6">
                                    {{ $present->name }}
                                </a>
                            </th>
            ...
            <div class="col-12">
                ...
                <table class="table">
                    ...
                    <tbody>
                        ...
                        <tr>
                            <th class="py-2" scope="row">
                                <a href="/admin/product/{{ $item->product_id }}/show" class="text-primary fw-bold fs-6">
                                    {{ $item->name }}
                                </a>
                            </th>
                ...
                <ul class="list-group">
                    <li class="list-group-item">
                        @if (( !$canceled && !$rejected )|| $order->status === "rejected" || $order->status === "canceled" )
                        <div class="row">
                            <div class="col-6 d-flex align-items-center">
                                <div class="alert alert-info m-0 w-100">
                                    {{__("app.".$order->status)}}
                                </div>
                            </div>
                            <div class="col-6 d-flex align-items-center">
                                <form action="/admin/order/{{$order->id}}/change-order-status" method="post">
                                    @csrf
                                    <select class="form-select" name="order_status">
                                        ...
                                        <option value="process"
                                            @if ($order->status === "process")
                                                selected
                                            @endif
                                        > {{__("app.process")}} </option>
                                        <option value="shipped"
                                            @if ($order->status === "shipped")
                                                selected
                                            @endif
                                        > {{__("app.shipped")}} </option>
                                        <option value="fullfied"
                                            @if ($order->status === "fullfied")
                                                selected
                                            @endif
                                        > {{__("app.fullfied")}} </option>
                                        <option value="rejected"
                                            @if ($order->status === "rejected")
                                                selected
                                            @endif
                                        > {{__("app.rejected")}} </option>
                                    </select>
                                    <div class="d-grid mt-2">
                                        <button class="btn-primary btn btn-sm w- 100">
                                            {{__("app.save")}}
                                        </button>
                                    </div>
                                </form>
                            ...
                            <div class="row align-items-center">
                                ...
                                <div class="col-4">
                                    <form action="/admin/order/{{$order->id}}/change-order-status" method="post">
                                        ...
                                        <div class="d-grid">
                                            <button class="btn btn-danger">{{__("app.cancel")}}</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
                            ...
                            <div class="row align-items-center">
                                ...
                                <div class="col-4">
                                    <form action="/admin/order/{{$order->id}}/change-order-status" method="post">
                                        ...
                                        <div class="d-grid">
                                            <button class="btn btn-warning">{{__("app.reject")}}</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
                    ...
                    <li class="list-group-item">
                        <h4 class="text-center">Returning Reasons</h4>
                        @foreach ($returned_orders as $item)
                            <p class="fw-bold text-muted">{{$item->reason}}</p>    
                        @endforeach
                    </li>
                    @endif
                    @if ($canceled_orders->isNotEmpty())
                    <li class="list-group-item">
                        <h4 class="text-center mb-3">Canceling Reasons</h4>
                        @foreach ($canceled_orders as $item)
                            <p class="fw-bold text-muted">{{$item->reason}}</p>
                        @endforeach
                    </li>
                    @endif
            ...
            <div class="col-12 mt-3">
                <div class="order-details-confirmation border-1 rounded-3">
                    ...
                    <ul class="order-details-form mb-4 p-3 p-md-0">
                        ...
                        <li>
                            <span>
                                <a href="/admin/product/{{ $cart_price->product_id }}/show" class="text-primary fw-bold">
