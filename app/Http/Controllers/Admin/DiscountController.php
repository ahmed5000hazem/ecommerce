...
class DiscountController extends Controller
{
    ...
    public function show($discount_type)
    {
        ...
        if ($discount_type === "all") {
            ...
            $products = Product::where("item_value_discount", 1)
            ->select("id", "name", "price")->get();
            $item_value = 1;
        } else if ($discount_type === "items-value") {
            $products = Product::where("items_value_discount", 1)
            ->select("id", "name")->get();
            $items_value = 1;
        } else if ($discount_type === "items-items") {
            $products = Product::where("items_items_discount", 1)
            ->select("id", "name")->get();
            $items_items = 1;
        } else{
            return redirect()->back();
        ...
        // return $products;
        return view("admin-views.discounts.show-discount", [
            "products" => $products,
            "all" => $all,
    ...
    public function edit($discount_type, $discount_id)
    {
        ...
        if ($discount_type === "item-value-disc") {
            ...
            return view("admin-views.discounts.edit-item-value", [
                "discount" => $discount,
                "product" => $product,
            ]);
            ...
            return view("admin-views.discounts.edit-items-value", [
                "discount" => $discount,
                "product" => $product,
            ]);
            ...
            return view("admin-views.discounts.edit-items-items", [
                "discount" => $discount,
                "product" => $product,
                "present" => $present,
            ]);
    ...
    public function delete ($discount_type, $discount_id, Request $request) {
        ...
        if ($discount_type === "item-value-disc") {
            ...
            return redirect("/admin/product/".$product."/show");
